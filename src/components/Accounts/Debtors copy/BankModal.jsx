        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <InputField
              label="Bank Name"
              value={bankForm.name}
              onChange={(e) => setBankForm(prev => ({ ...prev, name: e.target.value.toUpperCase() }))}
              required
            ...
            <InputField
              ...
              onChange={(e) => setBankForm(prev => ({ ...prev, swiftId: e.target.value.toUpperCase() }))}
            ...
            <InputField
              ...
              onChange={(e) => setBankForm(prev => ({ ...prev, accNo: e.target.value.toUpperCase() }))}
              placeholder="Enter account number"
            />
            <InputField
              label="Branch Code"
              value={bankForm.branchCode}
              onChange={(e) => setBankForm(prev => ({ ...prev, branchCode: e.target.value.toUpperCase() }))}
              placeholder="Enter branch code"
            />
            <InputField
              label="Purpose"
              value={bankForm.purpose}
              onChange={(e) => setBankForm(prev => ({ ...prev, purpose: e.target.value.toUpperCase() }))}
              placeholder="Enter purpose"
            />
            <InputField
              label="Country"
              value={bankForm.country}
              onChange={(e) => setBankForm(prev => ({ ...prev, country: e.target.value.toUpperCase() }))}
              placeholder="Enter country"
            />
            <InputField
              label="City"
              value={bankForm.city}
              onChange={(e) => setBankForm(prev => ({ ...prev, city: e.target.value.toUpperCase() }))}
              placeholder="Enter city"
            />
            <InputField
              label="Routing Code"
              value={bankForm.routingCode}
              onChange={(e) => setBankForm(prev => ({ ...prev, routingCode: e.target.value.toUpperCase() }))}
              placeholder="Enter routing code"
            />
            <TextareaField
              label="Address"
              value={bankForm.address}
              onChange={(e) => setBankForm(prev => ({ ...prev, address: e.target.value.toUpperCase() }))}
