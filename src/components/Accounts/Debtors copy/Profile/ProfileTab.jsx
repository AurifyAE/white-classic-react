...
const ProfileTab = ({
  ...
  const BalanceCard = ({ label, value, subValue }) => {
    ...
    const icon = isPositiveOrZero ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />;

    return (
      <div className={`rounded-xl p-4 shadow-sm border ${bgColor} border-gray-200 flex flex-col gap-1`}>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
        ...
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <p className="text-blue-100 text-sm">
              Account Code:{" "}
              <span className="font-semibold">{userData.acCode || "N/A"}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <BalanceCard
          ...
          value={formatCurrency(userData.balances?.cashBalance.amount, 'AED') || 0}
          color="text-red-600"
        />
        <BalanceCard
          label="Gold Balance"
          // value={userData.balances?.goldBalance?.totalValue
          //   ? `$${userData.balances.goldBalance.totalValue.toLocaleString()}`
          //   : "$0"}
          value={
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        >
          <div className="space-y-3">
            ...
            {/* <InfoRow label="Parent Group" value={userData.parentGroup} /> */}
            {/* <InfoRow label="Mode" value={userData.mode} /> */}
        ...
        >
          <div className="space-y-3">
            <InfoRow
              label="Account Type"
              value={userData.acDefinition?.accountType}
              highlight
            />
            ...
            <InfoRow
              label="Created Date"
              value={formatDateFunc(userData.createdAt)}
            />
            <InfoRow
              label="Last Updated"
              value={formatDateFunc(userData.updatedAt)}
            />
        ...
        >
          <div className="space-y-3">
            <InfoRow
              label="Document Type"
              value={userData.kycData?.documentType}
            />
            <InfoRow
              label="Document Number"
              value={userData.kycData?.documentNumber}
              highlight
            />
            <InfoRow
              label="Issue Date"
              value={formatDateFunc(userData.kycData?.issueDate)}
            />
            <InfoRow
              label="Expiry Date"
              value={formatDateFunc(userData.kycData?.expiryDate)}
            />
            <InfoRow
              label="Verification Status"
              value={
                userData.kycData?.isVerified ? "✅ Verified" : "❌ Not Verified"
              }
              highlight
            />
          </div>
        ...
        >
          <div className="space-y-3">
            <InfoRow
              ...
              value={userData.vatGstData?.registrationType}
            />
            <InfoRow
              label="VAT Number"
              value={userData.vatGstData?.registrationNumber}
              highlight
            />
            <InfoRow
              label="Registration Date"
              value={formatDateFunc(userData.vatGstData?.registrationDate)}
            />
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {userData.employees.map((employee, index) => (
                ...
                >
                  <div className="space-y-2">
                    <InfoRow label="Name" value={employee.name} highlight />
