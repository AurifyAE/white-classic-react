        ...
        <table className="w-full bg-white rounded-lg shadow-sm">
          ...
          <tbody>
            {filteredTableCurrencies.map((curr, index) => (
              <tr key={curr.currency} className={index % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                <td className="px-4 py-3 text-sm text-gray-600">{index + 1}</td>
                <td className="px-4 py-3 text-sm font-medium text-gray-800">{curr.currency}</td>
                <td className="px-4 py-3 text-sm text-gray-600">{curr.minRate}</td>
                <td className="px-4 py-3 text-sm text-gray-600">{curr.maxRate}</td>
                <td className="px-4 py-3 flex items-center space-x-2">
                  {/* Default currency checkmark */}
                  <input
                    type="radio"
                    name="defaultCurrency"
                    checked={selectedCurrency === curr.currency}
                    onChange={() => {
                      if (selectedCurrency === "AED" && curr.currency !== "AED") {
                        // Show Sonner confirmation here
                        // If confirmed, setSelectedCurrency(curr.currency)
                      } else {
                  ...
                  />
                  {/* Remove icon (except for AED) */}
                  {curr.currency !== "AED" && (
                    <button
                      onClick={() => setCheckedCurrencies(checkedCurrencies.filter(c => c !== curr.currency))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setBasicFormData({ ...basicFormData, remarks: e.target.value.toUpperCase() })
            }
            className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
...
const AcDefinitionTab = ({
  ...
  const [checkedCurrencies, setCheckedCurrencies] = useState(["AED"]);
  const [defaultCurrency, setDefaultCurrency] = useState("AED");

  // Initialize AED as default currency on component mount
  useEffect(() => {
    if (acDefinitionData.currency.currency == "" && currencyOptions.length > 0) {
  ...
  }, [currencyOptions, acDefinitionData, setAcDefinitionData]);

  const handleAddCurrency = (currencyCode) => {
    if (!checkedCurrencies.includes(currencyCode)) {
      setCheckedCurrencies((prev) => [...prev, currencyCode]);
    }
  };

  const handleRemoveCurrency = (currencyCode) => {
    if (currencyCode === "AED") return; // Prevent removing AED
    if (currencyCode === defaultCurrency) {
      toast.error("Cannot remove the default currency. Please set another currency as default first.");
      return;
    }
    setCheckedCurrencies((prev) => prev.filter((c) => c !== currencyCode));
  };

  const handleDefaultCurrencyChange = (newDefaultCurrency) => {
    if (newDefaultCurrency === defaultCurrency) return;

    // Show confirmation dialog
    const confirmChange = () => {
      toast.success(`Default currency changed to ${newDefaultCurrency}`, {
        action: {
          label: "Confirm",
          onClick: () => {
            setDefaultCurrency(newDefaultCurrency);
            setSelectedCurrency(newDefaultCurrency);
            const selectedCurrencyObj = currencyOptions.find((c) => c.currency === newDefaultCurrency);
            setAcDefinitionData({
              ...acDefinitionData,
    ...
    toast.warning(`You have already ${defaultCurrency} set as default currency. Do you need to change it to ${newDefaultCurrency}?`, {
      action: {
        label: "Yes, Change",
        onClick: () => {
          setDefaultCurrency(newDefaultCurrency);
  ...
  const handleCheckboxChange = (currency) => {
    if (currency === "AED") return; // Prevent unchecking AED
    setCheckedCurrencies((prev) =>
  ...
  const handleSearchChange = (val) => {
    ...
    if (matchedCurrency) {
      ...
      setSelectedCurrency(matchedCurrency.currency);
    }
  };
            ...
            <table className="w-full bg-white rounded-lg overflow-hidden shadow-sm">
              ...
              <tbody>
                {filteredTableCurrencies.map((curr, index) => (
                  <tr
                    key={curr.currency}
                    className={`${index % 2 === 0 ? "bg-gray-50" : "bg-white"} ${curr.currency === defaultCurrency ? "ring-2 ring-emerald-500 bg-emerald-50" : ""
                      }`}
                  >
                    <td className="px-4 py-3 text-sm text-gray-600">{index + 1}</td>
                    ...
                    <td className="px-4 py-3">
                      <input
                        ...
                        onChange={() => handleDefaultCurrencyChange(curr.currency)}
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        disabled={curr.currency === "AED"}
                        className={`p-1 rounded-full transition-colors ${curr.currency === "AED"
                          ? "text-gray-300 cursor-not-allowed"
                          : "text-red-500 hover:bg-red-100 hover:text-red-700"
                          }`}
                        title={curr.currency === "AED" ? "AED cannot be removed" : "Remove currency"}
                      >
                        <X className="w-4 h-4" />
                      </button>
                    </td>
    ...
    <div className="grid grid-cols-1 gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <input
          ...
          onChange={(e) =>
            setAddressData({ ...addressData, city: e.target.value.toUpperCase() })
          }
        ...
        <input
          ...
          onChange={(e) =>
            setAddressData({ ...addressData, country: e.target.value.toUpperCase() })
          }
          className="px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
          placeholder="Country"
        />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <input
          ...
          onChange={(e) =>
            setAddressData({ ...addressData, phoneNumber1: e.target.value.toUpperCase() })
          }
          className="px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
        ...
        <input
          ...
          onChange={(e) =>
            setAddressData({ ...addressData, phoneNumber2: e.target.value.toUpperCase() })
          }
          className="px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
        ...
        <input
          ...
          onChange={(e) =>
            setAddressData({ ...addressData, phoneNumber3: e.target.value.toUpperCase() })
          }
          className="px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <input
          ...
          onChange={(e) =>
            setAddressData({ ...addressData, email: e.target.value.toUpperCase() })
          }
          className="px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
          placeholder="Email"
        />
        <input
          ...
          onChange={(e) =>
            setAddressData({ ...addressData, telephone: e.target.value.toUpperCase() })
          }
          className="px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
...
const VATGSTTab = ({ vatGstData, setVatGstData, editingDebtor }) => {
  const handleFileChange = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-blue-50 to-cyan-50 p-6 rounded-xl border border-blue-100">
        ...
        <div className="mt-6">
          ...
          <div
            className={`border-2 border-dashed border-blue-300 rounded-xl p-6 text-center transition-colors ${editingDebtor ? "bg-gray-100 cursor-not-allowed" : "hover:border-blue-400"
              }`}
          >
            <input
              type="file"
            ...
            <label
              htmlFor="vat-gst-upload"
              className={`cursor-pointer flex flex-col items-center ${editingDebtor ? "pointer-events-none opacity-50" : ""
                }`}
            >
              <FileText className="w-8 h-8 text-blue-400 mb-2" />
              <p className="text-gray-600">
...
const KYCTab = ({ kycData, setKycData, selectedType = "account", editingDebtor }) => {
  const handleFileChange = (e) => {
    const file = e.target.files[0];
    if (!file) return;
            ...
            <div className="flex space-x-1 mt-4 bg-gray-100 rounded-xl p-1 shadow-sm">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`flex-1 py-3 px-4 rounded-lg font-medium transition-all duration-200 flex items-center justify-center space-x-2 ${activeTab === tab.id
