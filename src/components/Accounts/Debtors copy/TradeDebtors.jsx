...
const handleError = debounce((error, message, toastId) => {
  ...
  toast.error(message, {
    id: toastId, // Use a unique toastId to prevent duplicates
    duration: 4000,
  });
}, 500);
...
const fetchTradeDebtors = async (setTradeDebtors, setFilteredDebtors) => {
  try {
    ...
    const mappedTradeDebtors = data.map((debtor) => ({
      ...
      type: debtor.accountType || "DEBTOR", // Use accountType from data
    ...
    // Filter for DEBTOR type
    const Debtors = mappedTradeDebtors.filter((debtor) => debtor.type === "DEBTOR");

    setTradeDebtors(Debtors);
...
export default function TradeDebtors() {
  ...
  // useEffect(() => {
  //   fetchCurrencyMaster();
  //   fetchTradeDebtors(setTradeDebtors, setFilteredDebtors, setLoading);
  // }, []);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!debtorsSuccess || !currenciesSuccess) {
          ...
          handleError(new Error('Missing required fields'), "Some data couldn't be fetched. Please try again.");
        }
      } catch (error) {
    ...
    const fetchDataOnPageChange = async () => {
      ...
      try {
        ...
        if (!success) {
          ...
          handleError(new Error('Missing required fields'), "No trade debtors data available");
  ...
  //   setLoading(true);
  //   try {
  //     await axiosInstance.delete(`/account-type/${debtorId}`);
