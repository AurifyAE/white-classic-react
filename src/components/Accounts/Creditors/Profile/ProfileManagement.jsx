...
const orders = [
  ...
  //     try {
  //       setLoading(true);
  //       const response = await axiosInstance.get(`/trade-debtors/${debtorId}`);
  ...
  useEffect(() => {
    const fetchDebtor = async () => {
      ...
      try {
        setLoading(true);
        const response = await axiosInstance.get('/account-type?search=CREDITOR');
        const { data } = response.data;

        if (!Array.isArray(data) || data.length === 0) {
          setError("No creditor data found");
          setLoading(false);
          return;
        }

        // Find the specific debtor by ID
        const debtor = data.find((d) => d._id === debtorId || d.id === debtorId);
        ...
        // Find default currency or fallback to first currency
        const defaultCurrency = debtor.acDefinition?.currencies?.find(
        ...
        const mappedDebtor = {
          ...
          classification: debtor.classification || "",
          mode: debtor.mode || "",
          type: debtor.accountType || "CREDITOR",
          acCode: debtor.accountCode || "",
          createdAt: debtor.createdAt || "",
          updatedAt: debtor.updatedAt || "",
          balances: debtor.balances || {
          ...
          acDefinition: {
            accountType: debtor.accountType || "CREDITOR",
            branches: debtor.acDefinition?.branches || null,
            currencies: debtor.acDefinition?.currencies || [],
          },
          vatGstData: {
            ...
            registrationType: debtor.vatGstDetails?.vatStatus || "",
            registrationDate: debtor.vatGstDetails?.registrationDate || "",
            state: debtor.vatGstDetails?.state || debtor.addresses?.[0]?.city || "",
            documents: debtor.vatGstDetails?.documents || [],
          },
          kycData: debtor.kycDetails?.[0] || {
            documentType: "",
