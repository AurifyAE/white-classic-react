...
const ProfileTab = ({ userData, formatDate }) => {
  ...
  const BalanceCard = ({ label, value, currency, lastUpdated, isDefault }) => {
    ...
    const bgColor = isZero
      ...
      ? "bg-green-50"
      : "bg-red-50";
    const icon = isZero ? null : isPositiveOrZero ? (
    ...
    return (
      <div
        className={`rounded-xl p-4 shadow-sm border ${bgColor} border-gray-200 flex flex-col gap-1 relative`}
      >
        {isDefault && (
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-xs font-medium px-2 py-0.5 rounded-full ${color} bg-white border`}
          >
            {indicator}
          </span>
          {icon}
        ...
        {lastUpdated && (
          <div className="text-xs text-gray-400">
            Last Updated: {formatDateFunc(lastUpdated)}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
      ...
      <>
        <OrderStatementsTab userData={userData} />
      </>
    </div>
  );
};
