...
//update the display gold balance
  useEffect(() => {
  ...
  if (isOpen && orderToEdit) {
    const initializeEditMode = () => {
      ...
      setVoucherDetails({
        ...
        voucherType: orderToEdit.voucherType || "PUR",
...
// Add a separate useEffect for initial metal rates setup
useEffect(() => {
  if (isOpen && metalRates.length > 0 && !selectedMetalRate) {
    const gozRate = metalRates.find(rate => rate.rateType === "GOZ");
    if (gozRate) {
      setSelectedMetalRate(gozRate._id);
    } else {
...
}, [isOpen, metalRates, selectedMetalRate]);
...
useEffect(() => {
  ...
  const handleVolumeChange = (e) => {
    ...
    // Only update calculated value if user is editing volume field
 ...
 const handleValueChange = (e) => {
  ...
  const handleBuy = async () => {
    ...
    // Rest of your existing handleBuy code...
                    ...
                    >
                      <defs>
                        ...
                        <linearGradient
                          ...
                          x1="0"
                          y1="0"
                          x2="0"                Orders Placed Successfully
...
/>
            ...
            <div className="flex flex-col gap-4 mb-6 p-5">
              <div className="flex items-center gap-3">
                ...
                >
                  BUY at {formatWesternNumber(isBuyFrozen ? frozenBuyPrice : bidPrice)}
                </button>
                <button
