...
const PDFPreviewModal = ({ isOpen, onClose, purchase, onDownload }) => {
  ...
  const totalQuantityGm = purchase.orders.reduce((sum, order) => sum + (order.quantityGm || 0), 0);
  const totalPrice = purchase.orders.reduce((sum, order) => sum + (order.price || 0), 0);

  // Map all orders to table data
  const tableData = purchase.orders.map((order) => ({
  ...
  const goldRate = formatNumber(purchase.metalRateRequirements?.rate || purchase.orders[0]?.price || 0);
  const pureWeightGrams = formatNumber(totalQuantityGm * 1000);
  const signedBy = purchase.salesman || "AUTHORIZED SIGNATORY";

  return (
    <div className="fixed inset-0 z-40 bg-black/40 flex justify-center items-center px-4 py-6 overflow-auto">
      <div className="bg-white w-full max-w-3xl max-h-[90vh] rounded-lg shadow-lg text-black font-helvetica flex flex-col overflow-y-auto">
        ...
        <div className="relative text-center pt-2">
          ...
          <h2 className="text-[12px] font-bold text-right pr-3.5 mt-1">FIXING PURCHASE INVOICE</h2>
        ...
        <div className="flex mx-3.5 mt-1.5 text-[9px] border border-[#CDCDCD]">
          <div className="flex-1 p-2 border-r border-[#CDCDCD]">
            <p>Party Name: {purchase.partyId?.customerName || purchase.partyId?.accountCode || "N/A"}</p>
            <p className="mt-1.25">Phone: {purchase.partyPhone || "N/A"}</p>
            <p className="mt-1.25">Email: {purchase.partyEmail || "N/A"}</p>
          </div>
          <div className="flex-1 p-2">
            <p>PUR NO: {purchase.voucherNumber || "N/A"}</p>
            <p className="mt-1.25">Date: {new Date(purchase.createdAt).toLocaleDateString("en-GB", { day: '2-digit', month: 'long', year: 'numeric' })}</p>
        ...
        {/* Table */}
        <div className="overflow-x-auto px-3.5 mt-6">
          <table className="min-w-full border text-[8px] border-[#CDCDCD]">
            <thead className="bg-[#E6E6E6]">
              <tr>
                ...
                <th rowSpan="2" className="border border-[#CDCDCD] p-1 text-left align-middle">Payment Terms</th>
                <th rowSpan="2" className="border border-[#CDCDCD] p-1 text-right align-middle">Gold Bid Value</th>
                <th rowSpan="2" className="border border-[#CDCDCD] p-1 text-right align-middle">Quantity Gm</th>
                <th rowSpan="2" className="border border-[#CDCDCD] p-1 text-right align-middle">Price</th>
              </tr>
            </thead>
            <tbody>
              {tableData.map((row, idx) => (
                <tr key={idx} className="border-t border-[#CDCDCD]">
                  ...
                  <td className="border border-[#CDCDCD] p-1 text-left">{row.paymentTerms}</td>
                  <td className="border border-[#CDCDCD] p-1 text-right">{row.goldBidValue}</td>
                  <td className="border border-[#CDCDCD] p-1 text-right">{row.quantityGm}</td>
                  <td className="border border-r-0 border-[#CDCDCD] p-1 text-right">{row.price}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="px-3.5 py-2 text-[8px]">
          <div className="ml-auto border border-[#CDCDCD]" style={{ width: "33.33%" }}>
            <div className="flex border-b border-[#CDCDCD]">
              <div className="flex-1 border-r border-[#CDCDCD] p-1 font-bold text-center">Total Quantity</div>
              <div className="flex-1 p-1 text-center">{totals.totalQuantityGm}</div>
            </div>
            <div className="flex">
              <div className="flex-1 border-r border-[#CDCDCD] p-1 font-bold text-center">Total Price</div>
              <div className="flex-1 p-1 text-center">{totals.totalPrice}</div>
            </div>
          </div>
        </div>
        ...
        <div className="px-3.5 py-2 text-[8px]">
          ...
          <div className="border border-[#CDCDCD] mt-1">
            <div className="flex border-b border-[#CDCDCD]">
              <div className="w-[80px] border-r border-[#CDCDCD] p-1 font-bold">{formatNumber(totals.totalPrice)} CREDITED</div>
              <div className="flex-1 p-1 italic">{numberToDirhamWords(totals.totalPrice)}</div>
            </div>
            <div className="flex border-b border-[#CDCDCD]">
              <div className="w-[80px] border-r border-[#CDCDCD] p-1 font-bold">{pureWeightGrams} GMS CREDITED</div>
              <div className="flex-1 p-1 italic">{purchase.metalType?.rateType || "GOLD"} {pureWeightGrams} Gms</div>
            </div>
            <div className="p-1">Fixing purchase {pureWeightGrams} gm @</div>
          </div>
