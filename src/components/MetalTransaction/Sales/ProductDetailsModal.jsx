...
const ProductDetailsModal = ({
  ...
  const conversionRate = partyCurrencyValue || partyCurrency?.conversionRate || "1.00";
  ...
  const fetchMetalRates = useCallback(async () => {
    try {
      ...
      if (!editingItem) {
        const defaultRate =
          mappedMetalRates.find((rate) => rate.rateType === "GOZ") || mappedMetalRates[0];
        if (defaultRate) {
          setProductData((prev) => ({
            ...prev,
 ...
 useEffect(() => {
  ...
  const debouncedSetProductData = useCallback(
    debounce((newData) => {
      setProductData((prev) => {
        ...
        // Apply currency conversion factor for other calculations
        const conversionFactor = currencyCode === "AED" ? 1 : effectiveRate;

        // Define divisor for premium calculations
        const divisor =
          purityWeight > 0 && convFactGms > 0
            ? (parseFloat(purityWeight) / convFactGms) * convertrate
            : 0;
      ...
      if (lastEditedFields.metalRateRequirements === "rate") {
        ...
        if (lastEditedFields.makingCharges === "rate") {
          ...
          makingAmountStr = (parsedRate * grossWeight).toFixed(2);
        } else {
          const parsedAmount = parseFloat(makingAmountStr) || 0;
          makingRateStr = grossWeight > 0 ? (parsedAmount / grossWeight).toFixed(2) : "0.00";
        }
    ...
    [lastEditedFields, effectiveRate, currencyCode, editingItem, focusedFields]
  );
    ...
    (e) => {
      ...
      if (name.includes(".")) {
        ...
        setProductData((prev) => {
          ...
          const divisor =
            purityWeight > 0 && convFactGms > 0 ? (purityWeight / convFactGms) * convertrate : 0;
          ...
          if (parent === "makingCharges") {
            ...
            const parsedMakingAmount = parseFloat(updatedParent.amount) || 0;
            const parsedPremiumAmount = parseFloat(prev.premium.amount) || 0;
            updatedItemTotal.subTotal = (parsedMetalAmount + parsedMakingAmount + parsedPremiumAmount).toFixed(4);
      ...
      if (rawValue === "" || /^-?\d*\.?\d*$/.test(rawValue)) {
        setProductData((prev) => {
          ...
          if (field === "rate") {
            ...
            const rate = parseFloat(rawValue) || 0;
            updatedPremium.amount = divisor > 0 && appliedRate > 0 ? (divisor * rate * appliedRate).toFixed(2) : "0.00";
            ...
            const amount = parseFloat(rawValue) || 0;
            updatedPremium.rate = divisor > 0 && appliedRate > 0 ? (amount / (divisor * appliedRate)).toFixed(2) : "0.00";
          }
  ...
  const handleSave = useCallback(() => {
    ...
    const transformedData = {
      ...
      conversionRate: parseFloat(partyCurrency?.conversionRate) || 1,
      convertedAmounts: {
