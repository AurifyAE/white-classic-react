...
const SalesPDFPreviewModal = ({ isOpen, onClose, sale, onDownload }) => {
  if (!isOpen || !sale) return null;

  // Use correct fields from sale object
  const partyName = sale.partyName || 'N/A';
  const partyPhone = sale.partyPhone || 'N/A';
  const partyEmail = sale.partyEmail || 'N/A';
  const voucherNumber = sale.vocNo || sale.voucherNumber || 'N/A';
  const voucherDate = sale.vocDate ? new Date(sale.vocDate).toISOString().split('T')[0] : 'N/A';
  const paymentTerms = sale.paymentTerms || 'Cash';
  const salesman = sale.salesman || sale.createdBy?.name || 'N/A';

  // Process stock items
  const stockItems = sale.stockItems || [];
  ...
  const headingTitle = sale.fixed ? 'METAL SALE FIXING' : 'METAL SALE UNFIXING';
  const goldRate = formatNumber(sale.stockItems?.[0]?.metalRateRequirements?.rate || 0, 2);
  const signedBy = salesman || 'AUTHORIZED SIGNATORY';
  const pureWeightGrams = formatNumber(totals.totalPureWt, 3);
    ...
    <div className="fixed inset-0 z-40 bg-white/40 flex justify-center items-center px-4 py-6 overflow-hidden">
      <div className="bg-white w-full max-w-3xl max-h-[90vh] rounded-lg shadow-lg text-black font-helvetica flex flex-col overflow-y-auto">
        ...
        <div className="bg-gray-100 px-3.5 py-3 flex justify-end gap-4 border-t border-[#CDCDCD]">
          ...
          <button
            onClick={() => { onDownload(sale.id); onClose(); }}
