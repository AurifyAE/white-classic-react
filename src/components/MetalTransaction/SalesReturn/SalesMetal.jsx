        ...
        const mappedStockItems = (transaction.stockItems || []).map((item) => {
          ...
          return {
            ...
            otherCharges: {
              ...
              rate: Number(item.otherCharges?.percentage || item.otherCharges?.rate) || 0, 
              description: item.otherCharges?.description || "",
            },
            itemTotal: {
...
const handleSave = useCallback(async () => {
  ...
  try {
    ...
    showToast(error.response?.data?.message || "Failed to save metal sale return", "error");
  } finally {
  ...
  const handleExportAllToPDF = async () => {
    ...
    const salesTransactions = transactionData.filter(
      (transaction) => transaction.transactionType === "saleReturn"
    );
    ...
    showToast("Filtered sales return transactions exported to PDF successfully!", "success");
  };
...
{isDeleteModalOpen && (
  <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-100">
    <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
      ...
      <p className="text-sm text-gray-600 mb-6">
        Are you sure you want to delete this metal sale return? This action cannot be undone.
      </p>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h2 className="text-xl font-bold">
                          {editingStock ? "Edit Metal Sale Return" : "Add Metal Sale Return"}
                        </h2>
  ...
  >
    {isSaving ? (
      ...
      editingStock ? "Update Sale Return" : "Save Sale Return"
    )}
  </button>
</div>
                  </div>
