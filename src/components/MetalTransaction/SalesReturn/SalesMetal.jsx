...
export default function SalesReturnMetal() {
...
const handleDownloadPDF = async (purchaseId) => {
  ...
  const handleExportAllToPDF = async () => {
    ...
    try {
      ...
      const headingTitle = "METAL SALE RETURN METAL";
      doc.setFont("helvetica", "bold");
      doc.setFontSize(12);
  ...
  const handleAdd = useCallback(async () => {
    ...
    setIsModalOpen(true);
    toast.success(" new sales return Metal", {
      style: {
        background: "white",
        color: "green",
...
const handleEdit = useCallback(
  async (stock) => {
    ...
    try {
      ...
      let partyCurrencyData = { currencyCode: "AED", conversionRate: "1" }; // Default object
      let itemCurrencyData = { currencyCode: "AED", conversionRate: "1" }; // Default object
      ...
      setFormData({
        ...
        partyCurrency: partyCurrencyData, // Ensure this is an object
      });

      setIsModalOpen(true);
      showToast("Editing metal Sales Return", "success");
    } catch (error) {
...
const handleSave = useCallback(async () => {
  ...
  try {
    if (editingStock) {
      ...
      toast.success("Metal Sales return updated successfully!", {
        style: {
          background: "white",
          color: "green",
          border: "1px solid green",
      ...
      toast.success("Metal sales Return created successfully!", {
        style: {
          background: "white",
          color: "green",
          border: "1px solid green",
    ...
    setError(
      error.response?.data?.message || "Failed to save metal sales return"
    );
    toast.error(
      error.response?.data?.message || "Failed to save metal sales return",
      {
  ...
  const confirmDeletePurchase = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        showToast("Metal sales Return deleted successfully", "success");
            ...
            <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-100">
              <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
                ...
                <p className="text-sm text-gray-600 mb-6">
                  Are you sure you want to delete this metal sales Return? This
