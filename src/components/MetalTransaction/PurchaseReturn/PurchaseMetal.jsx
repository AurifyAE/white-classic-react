...
export default function purchasereturn() {
  ...
  const fetchMetalTransactions = useCallback(async () => {
    ...
    try {
      ...
      const purchaseTransactions = uniqueTransactions
        .filter((transaction) => transaction.transactionType === "purchaseReturn")
      ...
      setFilteredPurchase(purchaseTransactions); 
      setPagination(pag || {});
      setStockItems(
  ...
  const handleDownloadPDF = async (purchaseId) => {
    ...
    try {
      ...
      console.log("purchase return data", purchase.vatottucherNumber);
      ...
      // === HEADER SECTION ===
      const headingTitle = purchase.fixed ? 'METAL PURCHASE RETURN FIXING' : 'METAL PURCHASE RETURN UNFIXING';
  ...
  const handleExportAllToPDF = async () => {
    ...
    try {
      ...
      const headingTitle = 'METAL PURCHASE RETURN METAL';
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(12);
      doc.text(headingTitle, pageWidth - 14, logoY + logoHeight + 4, { align: 'right' });
      ...
      if (allStockItems.length === 0) {
        toast.error("No purchase Return transactions available to export");
        return;
      }
...
const handleEditStockItem = useCallback((item, index) => {
  ...
  setEditingStockIndex(index); // Make sure to set the index
...
const handleSave = useCallback(async () => {
  ...
  try {
    if (editingStock) {
      ...
      toast.success("Metal purchase return updated successfully!", {
        style: {
          background: "white",
          color: "green",
          border: "1px solid green",
      ...
      toast.success("Metal purchase return created successfully!", {
        style: {
          background: "white",
          color: "green",
          border: "1px solid green",
    ...
    setError(
      error.response?.data?.message || "Failed to save metal purchase return"
    );
    toast.error(error.response?.data?.message || "Failed to save metal purchase return", {
      style: {
        background: "white",
        color: "red",
        border: "1px solid red",
...
const confirmDeletePurchase = useCallback(async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      setFilteredPurchase((prev) => prev.filter((stock) => stock.id !== purchaseToDelete));
      toast.success("Metal purchase return deleted successfully", {
        style: {
          background: "white",
          color: "green",
          border: "1px solid green",
      ...
      <div className="bg-gradient-to-r from-blue-600 to-cyan-500 text-white rounded-xl p-6 mb-6 shadow-lg">
        <div className="flex flex-col sm:flex-row items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Metal Purchase Return Management</h1>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200/50">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan="8" className="px-6 py-12 text-center text-gray-500">
                        No metal purchases Return found
                      </td>
                    </tr>
                  )}
                </tbody>
  ...
  <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-100">
    <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
      ...
      <p className="text-sm text-gray-600 mb-6">
        Are you sure you want to delete this metal purchase return? This action cannot be undone.
      </p>
  ...
  >
    {isSaving ? (
      ...
      editingStock ? "Update Purchase Return" : "Save Purchase Return"
    )}
  </button>
</div>
                  </div>
                  ...
                  setTempStockItems((prevItems) => {
                    ...
                    if (itemIndex !== -1) {
                      ...
                      return prevItems.map((item, idx) =>
                        idx === itemIndex ? { ...productData } : item
                      );
                    } else {
                      // Prevent adding if stockId already exists (optional)
                      if (prevItems.some((item) => item.stockId === productData.stockId)) {
                        console.error("Duplicate stockId detected:", productData.stockId);
                        alert("Error: An item with this Stock ID already exists.");
                        return prevItems;
                      }
                      // Add new item
                      console.log("Adding new item:", productData);
