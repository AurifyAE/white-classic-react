...
import MetalPaymentPreviewModal from "./CurrencyPaymentPDFPreview";
// pdfMake.vfs = pdfFonts.vfs; // Correctly assign the vfs property
export default function CashPayment() {
  // const { module } = useParams();
  const module = window.location.pathname.split("/")[1] || "currency-payment";
...
const [isDeleting, setIsDeleting] = useState(false);
  ...
  const fetchAllVoucher = async () => {
    try {
      ...
      if (!selectedVoucher) {
        const defaultVoucher = voucherList.find((v) => v.module === "currency-payment");
        if (defaultVoucher) {
  ...
  const fetchAllCurrencyPayments = async () => {
    try {
      const response = await axiosInstance.get("/entry/cash-payments");
      setCurrencyPayments(response.data);
      return response.data; // Now returns the data for use in other places
    } catch (error) {
      console.error("Error fetching currency payments:", error);
...
const cashTypeOptions = useMemo(() => {
  ...
  const handleEdit = (payment) => {
    ...
    setVoucherType(payment.type || "cash payment");
    setPrefix("");
    ...
    const checkVoucher = async () => {
      ...
      if (voucher) {
        try {
          ...
          console.error("Error fetching currency payments:", err); // Fixed error message
