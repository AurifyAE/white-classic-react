...
const StockSearchModal = ({ isOpen, onClose, onSelect, formData, setFormData }) => {
  ...
  // Sample stock data
  const stocks = [
    { branchCode: "STK001", description: "Gold Ring", divisionCode: "DIV01" },
    { branchCode: "STK002", description: "Silver Chain", divisionCode: "DIV02" },
  ];

  const filteredStocks = stocks.filter(
    (stock) =>
      stock.branchCode.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <div className="w-full max-w-[600px] max-h-[80vh] overflow-y-auto rounded-2xl bg-white shadow-2xl">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-cyan-500 px-6 py-4 text-white">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold">Branch Code</h2>
            <button
              onClick={onClose}
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-gray-100">
                  <th className="px-4 py-2 text-left text-sm font-semibold text-gray-700">Branch Code</th>
              ...
              <tbody>
                {filteredStocks.map((stock) => (
                  ...
                  >
                    <td className="px-4 py-2 text-sm text-gray-700">{stock.branchCode}</td>
