...
const PurchaseFixCenter = () => {
  ...
  const fetchStockBalanceData = useCallback(async () => {
    ...
    try {
      ...
      setError("Failed to fetch purchase fix data: " + err.message);
  ...
  const summaryTotals = useMemo(() => {
    const {
      ...
      (acc, t) => {
        ...
        acc.totalCashValue +=
          (Number(t.cashDebit) || 0) - (Number(t.cashCredit) || 0);
        acc.totalGoldValue +=
          (Number(t.goldDebit) || 0) - (Number(t.goldCredit) || 0);
    ...
    const avgCashValue =
      totalTransactions > 0 ? totalCashValue / totalTransactions : 0;
    const avgGoldValue =
      totalTransactions > 0 ? totalGoldValue / totalTransactions : 0;
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-extrabold mb-2 tracking-tight">
                Purchase Fix Management
              </h1>
              <p className="text-blue-100 text-lg">
                Monitor and manage cash and gold purchase transactions
              </p>
            </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          ...
          <span className="text-gray-600">
            Loading purchase fix transactions...
          </span>
        </div>
      ) : (
        <>
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <button
                    className="flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-cyan-500 text-white rounded-lg hover:from-blue-700 hover:to-cyan-600 transition-all"
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search Purchase Fix Transactions..."
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    aria-label="Search purchase fix transactions"
                  />
                </div>
                ...
                <table className="w-full table-fixed border-collapse">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th
                        className="w-[10%] px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                        aria-label="Gold Debit"
                      >
                        Gold Debit
                      </th>
                      <th
                        className="w-[10%] px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                        aria-label="Gold Credit"
                      >
                        Gold Credit
                      </th>
                      <th
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap overflow-hidden text-ellipsis">
                          {transaction.cashDebit > 0 ? (
                            <div className="text-sm font-bold text-red-700">
                              {formatCommodityNumber(transaction.cashDebit, null)}
                            </div>
                          ) : (
                        ...
                        <td className="px-6 py-4 whitespace-nowrap overflow-hidden text-ellipsis">
                          {transaction.cashCredit > 0 ? (
                            <div className="text-sm font-bold text-green-700">
                              {formatCommodityNumber(transaction.cashCredit, null)}
                            </div>
                          ) : (
                            <div className="text-sm text-gray-300">—</div>
                          )}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap overflow-hidden text-ellipsis">
                          <div className="text-sm font-semibold text-blue-900">
                            {formatCommodityNumber(transaction.cashBalance, null)}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap overflow-hidden text-ellipsis">
                ...
                {!loading && transactionsWithBalance.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 text-lg">
                      No purchase fix transactions found matching your criteria.
                    </p>
                  </div>
                )}
              </div>
...
export default PurchaseFixCenter;
